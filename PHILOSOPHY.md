...
- **Simulated awards:** Because writing to a user's PDS requires their auth credentials (which a third-party app can't hold on their behalf without OAuth), award recording is simulated in localStorage. The data model and cryptographic schema are production-ready — the signing step is the only part not yet wired to a real PDS.
- **Progressive integration path:** When tangled.org is ready to merge, the frontend pages become tangled routes, the localStorage layer gets replaced with real XRPC writes, and the simulated signing gets replaced with the user's PDS-held private key.
